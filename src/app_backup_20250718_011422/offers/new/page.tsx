  ...
  const handleSendOffer = async () => {
    ...
    try {
      ...
      if (updateResult.success) {
        ...
        const offerLink = `${baseUrl}/offer/${createResult.offer.slug}`;
      ...
      <div className="w-full max-w-md mx-auto py-16 text-center">
        <div className="bg-green-50 border border-green-200 rounded-xl p-8 shadow">
          ...
          <div className="flex flex-col gap-3 mt-6">
            <Button onClick={() => router.push(`/patients/${selectedPatientId}`)} className="w-full">Hasta Profili'ne Git</Button>
            <Button onClick={() => router.push('/offers')} variant="outline" className="w-full">Teklifler Listesine Dön</Button>
            <Button onClick={() => router.push('/offers/new')} variant="ghost" className="w-full">Yeni Teklif Oluştur</Button>
          </div>
        </div>
      </div>
    );
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="text-center space-y-2">
                        <Button 
                          variant="outline" 
                          size="sm"
                          onClick={() => window.open('/pdf-templates', '_blank')}
                        >
                          Şablonları Yönet
                        </Button>
